...
export class FileService {
  /** 解析本地设备路径获取文件/目录对象 */
  public resolveFileSystem(filePath: string): Promise<Entry> {
  ...
  /** 读取文件 */
  public getFile(entry: Entry, fileName: string): Promise<File> {
    return new Promise((resolve, reject) => {
      entry.getFile(
        fileName,
  ...
  /** 写入文件 */
  public writeFile(file: File, data: string): Promise<boolean> {
  ...
  /** 读取目录下文件列表 */
  public readFileListFromDirectory(entry: Entry): Promise<Entry[]> {
    return new Promise((resolve, reject) => {
